...
(function(root) {
  ...
  // New equipment has a material vocabulary of its own. Inharmonic, rapidly
  // damped modes read as a metal plate/link, not another UI bell or laser note.
  const plate=(f,g,d=.15,at=0)=>bell(f,d,g,at);
  const link=(f,g,at=0)=>[click(f*1.8,g*.9,at),plate(f,g,.035,at),plate(f*1.61,g*.44,.026,at+.002)];
  set('weapon.shield.throw','후르르—잉 · 회전 원판','세 번 짧게 접히는 공기와 얇은 금속 가장자리의 울림으로 원판의 회전을 표현합니다.',[
    {...N(880,600,.29,.22,1.8,0,.008),filterPath:[[0,880],[.30,2200],[.67,1400],[1,600]],
      gainPath:[[0,0],[.10,.15],[.25,.065],[.40,.22],[.53,.07],[.67,.15],[.82,.045],[1,0]],noiseRateVariation:.06},
    N(3800,1800,.18,.037,2,.025,.016),plate(934,.014,.17,.018),plate(1541,.008,.12,.024),
  ]);
  set('weapon.shield.hit','텅—징! · 둥근 금속판','넓은 면의 단단한 접촉 뒤 서로 어긋난 금속 배음이 짧게 울립니다. 사슬추의 둔탁한 타격과 구별됩니다.',[
    {...N(2400,670,.065,.19,1.3,0,.001),gainPath:[[0,0],[.025,.19],[.16,.17],[.42,.07],[1,0]]},
    {...N(570,280,.105,.14,.7,0,.001,'lowpass'),gainPath:[[0,0],[.035,.14],[.22,.11],[.58,.045],[1,0]]},
    plate(458,.096,.21),plate(739,.050,.17,.001),plate(1267,.026,.105,.002),
  ]);
  set('weapon.shield.catch','착, 찰칵 · 다시 무장','손에 닿는 푹신한 접촉 뒤 손잡이의 작은 걸쇠가 닫힙니다. 긴 울림 없이 바로 끝납니다.',[
    N(1500,420,.046,.12,.8,0,.001,'lowpass'),wood(210,.036),
    ...link(1750,.024,.026),
  ]);
  set('weapon.shield.bounce','팅그! · 원판 벽 반사','방패 테두리가 벽을 치는 높고 짧은 금속 소리. 적을 때리는 낮은 텅 소리와 다릅니다.',[
    click(4600,.069),plate(1720,.057,.12),plate(2786,.025,.087,.001),
    {...N(2500,980,.056,.065,1.8,.008,.002),gainPath:[[0,0],[.035,.065],[.24,.055],[1,0]]},
  ]);
  set('weapon.flame.ignite','딸칵, 푸슉! · 연료 점화','분사 시작에만 밸브가 열리고 작은 불꽃이 붙습니다. 연속 분사 중에는 점화음을 반복하지 않습니다.',[
    click(3400,.060),click(4700,.030,.017),
    {...N(300,1500,.145,.21,.85,.025,.008,'lowpass'),filterPath:[[0,300],[.20,1200],[.57,1900],[1,780]],
      gainPath:[[0,0],[.15,.17],[.40,.21],[.76,.075],[1,0]]},
    N(3600,2400,.08,.027,.8,.030,.005,'highpass'),
  ]);
  set('weapon.flame.spray','슈아아— · 고압 불꽃','고압 노즐의 공기와 불규칙한 연소를 짧은 구간으로 이어 붙입니다. 음정 있는 모터음이나 매번 터지는 폭발음은 넣지 않았습니다.',[
    {...N(850,650,.235,.17,.8,0,.018,'lowpass'),filterPath:[[0,650],[.20,1200],[.43,850],[.65,1350],[1,650]],
      gainPath:[[0,0],[.08,.13],[.25,.17],[.40,.13],[.56,.16],[.75,.12],[.88,.11],[1,0]],noiseRateVariation:.13},
  ...
  set('weapon.chain.hit','크덩, 찰락! · 쇠추 충돌','무거운 추의 낮고 꽉 찬 충돌 뒤 느슨해진 사슬 고리가 두 번 흔들립니다. 검의 칼날 소리는 겹치지 않습니다.',[
    ...
    {...T(155,111,.12,.075,'triangle',0,.001),filter:{type:'lowpass',f:640,to:260,q:.7},
      gainPath:[[0,0],[.02,.075],[.16,.068],[.55,.029],[1,0]]},
    plate(813,.020,.060),...link(1760,.021,.039),click(2930,.029,.083),
  ]);
  set('skill.chain.swap','찰르륵—휙, 철컥! · 위치 교환','사슬이 당겨지는 빠른 금속 마찰, 반대로 스쳐 가는 바람, 팽팽하게 멎는 걸쇠로 교환의 앞뒤를 구별합니다.',[
    {...N(720,3100,.13,.18,2.2,0,.004),gainPath:[[0,0],[.15,.04],[.52,.10],[.85,.18],[1,0]]},
    {...N(2900,430,.21,.21,1.8,.10,.007),filterPath:[[0,2900],[.32,1300],[1,430]]},
